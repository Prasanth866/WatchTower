...
def test_websocket_rejects_unauthenticated():
    ...
    with pytest.raises(Exception):
        with client.websocket_connect("/api/v1/ws/crypto:btc") as ws:
            pass
...
def test_websocket_accepts_authenticated_and_receives_broadcast():
    ...
    with TestClient(app) as client:
        with client.websocket_connect(f"/api/v1/ws/crypto:btc?token={token}") as ws:
            ...
            # Broadcast the event
            asyncio.run(manager._broadcast("crypto:btc", event))
            
            # Receive and assert the broadcasted event
            data = ws.receive_json()
            assert data["topic"] == "crypto:btc"
